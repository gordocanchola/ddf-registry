...
public class RegistryRestEndpoint {
    ...
    public Response publish(@PathParam("registryId") String registryId,
        ...
        String dest;
        try {
            dest = URLDecoder.decode(destinationId, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity("Could not decode source id")
                    .build();
        }

        try {
            publicationHelper.publish(registryId, dest);
        } catch (FederationAdminException e) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity("Error publishing node to destination " + dest)
                    .build();
        }
    ...
    public Response unpublish(@PathParam("registryId") String registryId,
        ...
        try {
            publicationHelper.unpublish(registryId, dest);
        } catch (FederationAdminException e) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity("Error publishing node to destination " + dest)
                    .build();
    ...
    public void setPublicationHelper(RegistryPublicationHelper publicationHelper) {
